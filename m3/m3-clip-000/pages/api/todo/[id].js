...
export default async function userHandler(
  ...
  const id =
    ids.length === 1
      ? ids[0]
      : undefined; // likely never happen because if no value, then the URL goes to GET instead

  async function getTodosData() {
    const readFileData = await readFile(
      jsonFile,
    );
